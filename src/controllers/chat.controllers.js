...
export const getChatMessages = asyncHandler(async (req, res) => {
    ...
    // - Messages deleted "for everyone" are INCLUDED but with deletedForEveryone: true
    //   so frontend can show "This message was deleted"
    // - Messages deleted "for me" are EXCLUDED (only hidden from this user)
    const messageQuery = {
        chatId,
        // Only exclude messages deleted for this specific user (delete for me)
    ...
    const [messages, totalMessages] = await Promise.all([
        Message.find(messageQuery)
            ...
            .skip(skip)
            .limit(pageLimit)
            .select('sender message messageType mediaUrl fileName fileSize duration timestamp readBy replyTo reactions deletedForEveryone deletedForEveryoneAt')
            .populate('sender', 'username fullName profileImageUrl')
            .populate({
                path: 'replyTo',
                select: 'message sender timestamp deletedForEveryone',
                populate: {
                    path: 'sender',
